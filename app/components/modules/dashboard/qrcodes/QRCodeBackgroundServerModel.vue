...
const backgroundStyle = ref<Record<string, string>>({
  backgroundImage: `url('${apiImageUrl}${props.bodaEvent.qrCodeImage_Url}')`,
...
const tokens = props.bodaEvent.name.split(' ');
...
<template>
  ...
  >
    <div class="mx-2 flex justify-between">
      ...
      >
        <p class="font-script text-lg">{{ bodaEvent.eventTypeName }} de</p>
    ...
    >
      <Qrcode
        :value="`${bodaEvent.initials}-${guest.localId} - ${guest.name} - Mesa: ${guest.deskName}`"
        variant="pixelated"
        width="200"
        height="200"
      />
